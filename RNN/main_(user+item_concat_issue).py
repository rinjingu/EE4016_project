...
class RecommenderRNN(nn.Module):
    def __init__(self, user_input_size, item_input_size, hidden_size, output_size):
        ...
        self.user_embedding = nn.Embedding(user_input_size, hidden_size)
        self.item_embedding = nn.Embedding(item_input_size, hidden_size)

        self.user_rnn = nn.RNN(hidden_size, hidden_size, batch_first=True)
        self.item_rnn = nn.RNN(hidden_size, hidden_size, batch_first=True)
    ...
    def forward(self, user_input, item_input):
        ...
        _, user_output = self.user_rnn(user_embedded)
        _, item_output = self.item_rnn(item_embedded)

        user_representation = user_output[:, -1]  # Using only the last output from user RNN
        item_representation = item_output[:, -1]  # Using only the last output from item RNN

        concatenated = torch.cat((user_representation, item_representation), dim=1)
